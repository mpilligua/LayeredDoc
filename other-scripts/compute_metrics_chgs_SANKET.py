...
def count_psnr():
    ...
    gt_folder   = "/ghome/mpilligua/DocumentConversion/Data/NewTest/L0"
    pred_l0 = "/ghome/mpilligua/DocumentConversion/Results_paper/Inferencia/Restormer-6Ch/Millor_l0/OurTest/Real_Denoising/"
    
    gt_imgs = os.listdir(gt_folder)
    flipped = False
    for im in gt_imgs:
        print(gt_folder+'/'+im)
        print(pred_l0 +  im.replace('.png', '_l0.png'))
        
        gt_image = cv2.imread(gt_folder+'/'+im)
        max_p =  np.max(gt_image) # max_p is 1 or 255
        gt_image = gt_image / max_p
        pred_image = pred_l0 +  im.replace('.png', '_l0.png')
        pred_image = cv2.imread(pred_image)

        total_psnr+=psnr(pred_image,gt_image)
        qo+=1

        print("PSNR for image ",im," is ",psnr(pred_image,gt_image))
        #cv2.imwrite
        #
